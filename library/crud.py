...
from .models import Book, Author, User, Payment, Subscription
from .schemas import (
    CreateBook, CreateAuthor, UpdateBook, UpdateAuthor, UserCreate, SubscriptionCreate, UserUpdate, PaymentCreate
)
...
def subscription_update(db: Session, subscription_id: int, subscription_data: SubscriptionCreate):
...
def subscription_delete(db: Session, subscription_id: int):
    db_subscription = db.query(Subscription).filter(Subscription.id == subscription_id).first()
    if db_subscription:
